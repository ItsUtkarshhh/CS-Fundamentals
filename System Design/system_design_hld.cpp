...
//           : The big analogy : The Global Postal System - Imagine a world where there are no standardized rules for mail. If you want to send a letter from the US to Japan.
...
// Why Is It Used? (Why it matters) : Without the OSI model, networking would be a chaotic mess. It is used for three massive reasons.
...
// 7 Layers of OSI Model : To understand how data flows, it helps to look at the layers. When you send data, it travels from Layer 7 down to Layer 1. When data is received, it travels from Layer 1 up to Layer 7.
//                       : How sending a message actually works through OSI model - OSI model has 7 layers which are, Application layer, Presentation Layer, Session Layer, Transport Layer, Network layer, Data Layer, Physical layer etc.
//                       : OSI Model : Application Layer - Human-computer interaction, where applications access network services. Example :  Web browsers, Email clients, Hypertext Transfer Protocol (HTTP + S), File Transfer Protocol (FTP), SMTP, DNS etc.
//                                   : Presentation Layer - This Data formatting, encryption, decryption, and compression on sender side, and does the exact opposite on the receiver side to make the data readable.
//                                   : Session Layer : Establishes, manages, and terminates connections (sessions) between applications. Example : Remote Procedure Call (RPC), APIs etc
//                                   : Transport Layer : End-to-end communication, flow control, error checking, and segmentation. Example : TCP, UDP etc.
//                                   : Network Layer : Routing data packets across different networks - logical addressing. Means, it determines the best physical path for data and routes it across different networks. Example : IPv4, IPv6, ICMP, Routers
//                                   : Data Link Layer : Handles data transfer between devices on the same local network and detects errors in the physical layer - Physical Addressing. Example : MAC addresses, Ethernet, network switches.
//                                   : Physical Layer : Transmission of raw unstructured bit streams over a physical medium. Example : Cables, fiber optics, hubs, wireless frequencies etc.
//                       : Real World Working : The Microsecond Software Prep - The moment you hit "Send," Layers 7 down to 2 process the data inside your device's memory almost instantly.
//                                                                            - Your text is formatted, encrypted, chopped into pieces, and stamped with your IP address and MAC address. This entire packaging process happens before a single bit leaves your device.
//                                            : The Route and IP Addresses - IP Addresses are pre-decided - Your device already knows its own IP address and the receiver's IP address before sending.
//                                                                         - The Route is NOT pre-decided - Layers 3 and 2 do not lock in a specific path across the internet. Instead, your device just packs the data and hands it to your home router. As the data travels across the internet, each router it hits looks at the IP address and decides the next best turn in real time.
//                                            : The Physical Transmission - Once Layers 7-2 finish packing the data into digital bits (0s and 1s), Layer 1 immediately takes over. Your network card instantly converts those bits into electrical pulses (ethernet cable), light flashes (fiber optics), or radio waves (Wi-Fi) to shoot them out into the world.
//                                            : How data actually reaches the end user : Gathering the Addresses before sending.
//                                                                                     : Your IP & Your MAC : Your computer already knows these. Your MAC is permanently burned into your network card, and your local IP was given to your device by your router when you connected to the Wi-Fi
//                                                                                     : Your Friend's IP : Found via a DNS (Domain Name System) lookup. If you type a website name or look up a friend's device ID, your computer asks a DNS server, which replies with your friend's exact IP address.
//                                                                                     : Your Friend's MAC : Unknown. Because your friend is on a different network, your computer relies on ARP (Address Resolution Protocol) to shout out locally: "Who has the IP of my local router? Give me your MAC address!
//                       : Modern Developments in OSI & Networking : In modern, practical computer networking, we don't actually use the OSI model to build networks. Instead, we use the TCP/IP Model (also known as the Internet Protocol Suite).

// TCP/IP Model : While the OSI model is an amazing theoretical blueprint, TCP/IP is the actual real-world implementation. Every time you send a WhatsApp message, stream a video, or browse a website, you are using the TCP/IP model.
//              : Why Did the Change Happen? (OSI vs TCP/IP) : It was not a complete, ground-up change in how data works, but it is a massive structural change in philosophy.
//                                                           : The OSI model didn't "fail" because it was broken; it lost the race to TCP/IP due to timing, complexity, and politics. Here is why the industry shifted:
//                                                           : Reasons : Practical vs. Theoretical Origins : OSI was created by a committee (ISO) in 1984. They sat in a room and designed the perfect, most detailed theoretical model before actually building the software and protocols for it. It was top-heavy and over-engineered.
//                                                                                                         : TCP/IP was created by engineers and researchers (funded by the US US Department of Defense's DARPA) in the 1970s. They actually built the protocols, made them work in the real world (on the ARPANET), and then wrote the model based on what worked.
//                                                                     : Over-Segmentation in the OSI Model : In the OSI model, Layers 5 (Session), 6 (Presentation), and 7 (Application) are separate. In the real world, separating encryption (Layer 6) from the web browser (Layer 7) created unnecessary computing overhead. TCP/IP wisely collapsed these three layers into a single, highly efficient Application Layer.
//                                                                                                          : Similarly, OSI's Layer 1 (Physical) and Layer 2 (Data Link) were combined in TCP/IP because hardware manufacturers almost always design the physical port (like an Ethernet port) and the data link software (like Ethernet protocols) to work together as a single unit.
//                                                                     : Market Timing : By the time the OSI model was officially finalized and ready for implementation in the mid-to-late 1980s, the TCP/IP suite was already running the foundational infrastructure of the internet. Changing everything to match the OSI model would have cost billions of dollars for no practical performance gain.
